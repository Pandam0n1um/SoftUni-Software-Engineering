from project.common.drink_factory import DrinkFactory
from project.common.food_factory import FoodFactory
from project.common.table_factory import TableFactory
...
class Bakery:
    ...
    def order_food(self, table_number: int, *food_names: []):
        ...
        non_ordered_foods = f'{self.name} does not have in the menu:\n'
        for food_name in food_names:
            food = self.find_food_by_name(food_name)
            if food is None:
                non_ordered_foods += food_name + '\n'
            else:
                table.order_food(food)
                ordered_foods += str(food) + '\n'
        return ordered_foods + non_ordered_foods.strip()
    ...
    def order_drink(self, table_number: int, *drink_names: []):
        ...
        non_ordered_drinks = f'{self.name} does not have in the menu:\n'

        for drink_name in drink_names:
            drink = self.find_drink_by_name(drink_name)
            if drink is None:
                non_ordered_drinks += drink_name + '\n'
            else:
                table.order_drink(drink)
                ordered_drinks += str(drink) + '\n'
        return ordered_drinks + non_ordered_drinks.strip()
    ...
    def get_free_tables_info(self):
        free_tables=[table.free_table_info() for table in self.tables_repository if not table.is_reserved]
        result='\n'.join(free_tables)
        return result.strip()
