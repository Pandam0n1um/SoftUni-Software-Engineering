...
class TestIntegerList(TestCase):
	...
	def test_add_non_integer_is_added_raises(self):
	...
	def test_instert_element_with_not_valid_index_raises(self):
		with self.assertRaises(IndexError) as ex:
			self.list_integers.insert(3, 50)
		self.assertEqual("Index is out of range", str(ex.exception))

	def test_insert_non_integer_is_added_raises(self):
		with self.assertRaises(ValueError) as ex:
